...
def run(tau): 
    ...
    n_hidden = int(np.loadtxt(folder + '/n_hidden.txt').tolist())

    # We load the number of training epocs

    n_epochs = int(np.loadtxt(folder + '/n_epochs.txt').tolist())

    # We load the indexes for the features and for the target

    index_features = np.loadtxt(folder + '/index_features.txt').astype(int)
    index_target = np.loadtxt(folder + '/index_target.txt').astype(int)
