...
#call(["rm", "results/test_ll.txt"])
#call(["rm", "results/test_rmse.txt"])
#call(["rm", "results/test_MC_rmse.txt"])
#call(["rm", "results/time.txt"])
#call(["rm", "results/log.txt"])
...
for i in range(n_splits):
    ...
    network = net.net(X_train, y_train,
        [ n_hidden ], normalize = True, n_epochs = 40 , X_test=X_test, y_test=y_test) #int(n_epochs*1-20)
